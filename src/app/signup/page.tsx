  ...
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await authClient.login({
        ...
        onSuccess: () => {
          console.log("User signed up or logged in successfully");
    ...
    <div className="flex h-screen items-center justify-center p-4 bg-gray-100 dark:bg-gray-900 transition-colors duration-300">
      <div className="w-full max-w-md p-8 bg-white dark:bg-gray-800 shadow-md rounded-lg">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 text-center">
          Sign Up or Log In to ChainAuth
        </h2>
        <form className="mt-8 space-y-6" onSubmit={handleSignup}>
          <div>
            <input
              type="email"
              required
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md dark:bg-gray-700 dark:text-white transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
